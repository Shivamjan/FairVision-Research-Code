"""
Training script for PL-CQC.

Usage examples:
    # Fitzpatrick17k
    Run this code as: 

        python train_pl_cqc.py \
...
def train_model(label, dataloaders, device, dataset_sizes, model,
    ...
    for epoch in range(num_epochs):
        ...
        for phase in ['train', 'val']:
            ...
            for n_iter, batch in enumerate(loop):
                ...
                if phase == 'train':
                    ...
                    with torch.set_grad_enabled(phase == 'train'):
                        ...
                        if epoch < 2:
                            l_gqvk, loss_dict = lg_cqc_loss_v2(
                        ...
                        loss_cls_inf = criterion[0](output[17], label_c)
                        ...
                        gate_conf_sum += loss_dict['avg_confidence']
                        gate_weight_sum += loss_dict['avg_gate_weight']
                        gate_count += 1
                    ...
                    if phase == 'train':
                        ...
                        writer.add_scalar('Loss/' + phase + '/lg_cqc', loss_dict['lg_cqc_conditional'], train_step)

                        writer.add_scalar('Accuracy/' + phase,
                                          (torch.sum(preds == label_c.data)).item() / inputs.size(0), train_step)
                        writer.add_scalar('Balanced-Accuracy/' + phase,
                                          balanced_accuracy_score(label_c.data.cpu(), preds.cpu()), train_step)
                        train_step += 1
            ...
            # NEW!!!!
            if phase == 'train' and gate_count > 0:
                avg_conf_epoch = gate_conf_sum / gate_count
                avg_gate_epoch = gate_weight_sum / gate_count
                print(f"Epoch {epoch}: Avg confidence={avg_conf_epoch:.3f}, Avg gate weight={avg_gate_epoch:.3f}")
                writer.add_scalar('Metrics/train_epoch/avg_confidence', avg_conf_epoch, epoch)
                writer.add_scalar('Metrics/train_epoch/avg_gate_weight', avg_gate_epoch, epoch)
...
if __name__ == '__main__':
    ...
    for indexer, label in enumerate([level]):
        ...
        writer = SummaryWriter(
            comment="logs_{}_{}_{}_{}_{}.pth".format(model_name, n_epochs, label, holdout_set, seed))
        print(label)
        ...
        torch.save(model_ft.state_dict(),
                       path_op + '/' + "model_path_{}_{}_{}_{}_{}_{}.pth".format(model_name, n_epochs, label,
                                                                                 holdout_set, dataset_name, seed))
        torch.save(model_ft, path_op + '/' + "model_path_{}_{}_{}_{}_{}_{}.pt".format(model_name, n_epochs, label,
                                                                                          holdout_set, dataset_name,
                                                                                          seed))
        print("gold")
        training_results.to_csv(
                path_op + '/' + "training_{}_{}_{}_{}_{}_{}.csv".format(model_name, n_epochs, label, holdout_set,
                                                                        dataset_name, seed))
        ...
        with torch.no_grad():
            ...
            for i, batch in enumerate(dataloaders['val']):
                ...
                running_balanced_acc_sum += balanced_accuracy_score(classes.data.cpu(), preds.reshape(-1).cpu()) * \
                                            inputs.shape[0]
        ...
        if label == "low":
            ...
            df_x = pd.DataFrame({
                    ...
                    "prediction_probability": flatten(p_list),
                    "prediction": flatten(prediction_list)})
        df_x.to_csv(path_op + '/' + "results_{}_{}_{}_{}_{}_{}.csv".format(model_name, n_epochs, label, holdout_set,
                                                                               dataset_name, seed),
                        index=False)
