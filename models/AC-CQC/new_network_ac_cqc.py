...
def ac_cqc_loss(model_outputs,
    ...
    # ============================================
    # Fairness Loss (Attention Consistency)
    # ============================================
    if metric == 'l1':
        ...
        kl_true = F.kl_div(
            (m + eps).log(), attn_text + eps,
            reduction='batchmean', log_target=False
        )
        kl_cf = F.kl_div(
            (m + eps).log(), attn_cf + eps,
            reduction='batchmean', log_target=False
        )
    ...
    if include_contrastive:
        ...
        logits_true = torch.matmul(V_true_norm, T_true_norm.T) / DEFAULT_TAU
        ...
        logits_cf = torch.matmul(V_cf_norm, T_cf_norm.T) / DEFAULT_TAU
    ...
    # Total Loss
    # ============================================
    total_loss = (lambda_fair * loss_fairness) + loss_contrastive
